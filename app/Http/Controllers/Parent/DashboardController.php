...
class DashboardController extends Controller
{
    ...
    public function index()
    {
        ...
        $childrenSummaries = $children->map(function ($student) use ($session) {
            $cacheKey = "parent_child_summary_{$student->id}_" . Carbon::today()->toDateString() . "_v1";

            return Cache::remember($cacheKey, 300, function () use ($student, $session) {
                ...
                $todayAttendance = $student->attendances()
                    ...
                    ->map(function ($attendance) {
                        ...
                        $isLab = $this->isLabSession($sess->period ?? null, $sess->subject->type ?? null);

                        return [
                            'subject_name' => $sess->subject->name,
                            'subject_code' => $sess->subject->code,
                            'period'       => $sess->period,
