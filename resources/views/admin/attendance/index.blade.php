    ...
    <section class="grid gap-6 xl:grid-cols-[1.35fr_0.95fr]">
        <div class="space-y-6">
            <article class="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm">
                <div class="flex flex-wrap items-end justify-between gap-3">
                    <div>
                        <p class="text-[11px] font-bold uppercase tracking-[0.18em] text-slate-400">Daily Attendance Trend</p>
                        <h2 class="mt-1 text-2xl font-black tracking-tight text-slate-950">Last 30 days</h2>
                        <p class="mt-1 text-sm text-slate-500">A smooth line with subtle fill for attendance movement.</p>
                    </div>
                    <span class="rounded-full bg-rose-50 px-3 py-1.5 text-xs font-bold text-[#8B0000]">{{ $charts['dateRangeLabel'] }}</span>
                </div>
                <div class="mt-5 h-[340px] rounded-[1.5rem] bg-slate-50/80 p-4 ring-1 ring-slate-200/80">
        ...
        <div class="space-y-6">
            <article class="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm">
                <div class="flex items-end justify-between gap-3">
                    <div>
                        <p class="text-[11px] font-bold uppercase tracking-[0.18em] text-slate-400">Department Comparison</p>
                        <h2 class="mt-1 text-xl font-black tracking-tight text-slate-950">Average attendance by department</h2>
                    </div>
                    <span class="rounded-full bg-slate-100 px-3 py-1.5 text-xs font-bold text-slate-700">Horizontal bar chart</span>
                </div>
                <div class="mt-4 h-[240px] rounded-[1.5rem] bg-slate-50/80 p-4 ring-1 ring-slate-200/80">
            ...
            <article class="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm">
                <div class="flex items-end justify-between gap-3">
                    <div>
                        <p class="text-[11px] font-bold uppercase tracking-[0.18em] text-slate-400">Semester Distribution</p>
                        <h2 class="mt-1 text-xl font-black tracking-tight text-slate-950">Multi-semester running</h2>
                    </div>
                    <span class="rounded-full bg-violet-50 px-3 py-1.5 text-xs font-bold text-violet-700">Donut chart</span>
                </div>
                <div class="mt-4 h-[240px] rounded-[1.5rem] bg-slate-50/80 p-4 ring-1 ring-slate-200/80">
                    <canvas id="semesterDistributionChart"></canvas>
                </div>
                <div class="mt-4 flex flex-wrap gap-2 text-xs font-semibold text-slate-500">
                    @forelse($charts['semesterDistribution']['rows'] as $row)
                        <span class="rounded-full bg-slate-100 px-3 py-1.5">{{ $row['label'] }} · {{ number_format($row['rate'], 1) }}%</span>
                    @empty
                        <span class="rounded-full bg-slate-100 px-3 py-1.5">No semester data yet</span>
                    @endforelse
                </div>
            </article>
        </div>
                            ...
                            @forelse($attendanceSessions as $session)
                                ...
                                <tr class="group transition hover:bg-slate-50/70">
                                    <td class="px-4 py-3.5">
                                        <p class="font-semibold text-slate-900">{{ bsDate($session->date, 'd M Y') ?: '—' }}</p>
                    ...
                    <table class="min-w-full divide-y divide-slate-100 text-sm">
                        <thead class="sticky top-0 bg-slate-50/95 backdrop-blur">
                            <tr class="text-[11px] font-bold uppercase tracking-[0.18em] text-slate-400">
                                ...
                                <th class="px-4 py-3 text-left">30-day sparkline</th>
                                <th class="px-4 py-3 text-left">Absent</th>
                                ...
                                <tr class="group transition hover:bg-slate-50/70">
                                    ...
                                    <td class="px-4 py-3.5">
                                        <svg viewBox="0 0 96 28" class="h-8 w-24 text-[#8B0000]">
                                            <path d="{{ $row['sparkline'] }}" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"></path>
                                        </svg>
                                    </td>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const trendData = @json($charts['attendanceTrend']);
    const departmentData = @json($charts['departmentComparison']);
    const semesterData = @json($charts['semesterDistribution']);
    ...
    if (trendCanvas) {
        ...
        gradient.addColorStop(0, 'rgba(139, 0, 0, 0.20)');
    ...
    const semesterCanvas = document.getElementById('semesterDistributionChart');
    if (semesterCanvas) {
