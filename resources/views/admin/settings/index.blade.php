    ...
    <div class="flex flex-col lg:flex-row gap-6" x-data="{ activeTab: 'profile', hasChanges: false }">
        ...
        <main class="min-w-0 flex-1">
            ...
            <div x-show="activeTab === 'profile'" x-cloak class="space-y-6">
                <form action="{{ route('admin.settings.profile.update') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                    @csrf
                    @method('PATCH')
                                    ...
                                    <div class="mt-4 text-center">
                                        ...
                                        <div class="mt-4 space-y-2">
                                            <div class="flex items-center justify-between rounded-lg bg-white px-3 py-2 text-xs">
                                                ...
                                                <span class="font-semibold text-slate-900">{{ $user->roles->first()?->name ?? 'N/A' }}</span>
                                            </div>
                                            @if($user->teacher?->department)
                                            <div class="flex items-center justify-between rounded-lg bg-white px-3 py-2 text-xs">
                                                <span class="text-slate-500">Department</span>
                                                <span class="font-semibold text-slate-900">{{ $user->teacher->department->name }}</span>
                                            </div>
                                            @endif
                                        </div>
                                    </div>
            ...
            <div x-show="activeTab === 'security'" x-cloak class="space-y-6">
                ...
                <div class="rounded-xl border border-slate-200/80 bg-white shadow-sm">
                    ...
                    <form action="{{ route('admin.settings.password.update') }}" method="POST" class="p-6">
                        @csrf
                        @method('PATCH')
                ...
                <div class="rounded-xl border border-slate-200/80 bg-white shadow-sm">
                    ...
                    <div class="border-t border-slate-100 bg-slate-50 px-6 py-4">
                        <form action="{{ route('admin.settings.logout-all') }}" method="POST" x-data @submit.prevent="
