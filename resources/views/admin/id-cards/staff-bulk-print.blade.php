...
@php
    ...
    $qrBase64    = $qrMap[$staff->id] ?? null;

    // Barcode — flex div bars matching the preview card logic
    $barHtml = '';
    if (in_array($barcodeType, ['both', 'barcode'])) {
        $sno    = $staff->staff_code ?: str_pad((string) $staff->id, 8, '0', STR_PAD_LEFT);
        ...
        <div style="background:white; position:relative; padding-top:58px;">
            ...
            <div style="text-align:center; padding:4px 14px 6px;">
                ...
                <div style="font-size:10.5px;font-weight:800;color:#1a1a1a;text-transform:uppercase;margin-top:2px;letter-spacing:0.8px;">
                    {{ mb_strtoupper($designation ?? '') }}
                </div>
            </div>

            {{-- Details --}}
            <div style="padding:2px 16px 6px; font-size:10px; color:#1e293b; line-height:1.85; text-align:center;">
                <div><span style="color:#475569;">Staff Code:</span> &nbsp;<strong>{{ $staffCode }}</strong></div>
                @if($department)
                    <div><span style="color:#475569;">Department:</span> &nbsp;<strong>{{ $department }}</strong></div>
                @endif
                @if($address)
            ...
            <div style="padding:8px 14px; display:flex; justify-content:space-between; align-items:flex-end; gap:6px;">
                ...
                <div style="flex:1;min-width:0;">
                    ...
                    <div style="font-size:7px;text-align:center;margin-top:2px;font-family:monospace;letter-spacing:1px;">{{ $staffCode }}</div>
                </div>
                @endif
                ...
                <div style="flex-shrink:0;">
                    ...
                    @else
                        <img src="https://api.qrserver.com/v1/create-qr-code/?size=55x55&data={{ urlencode($staffCode) }}"
