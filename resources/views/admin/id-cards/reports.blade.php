...
<div x-data="reportPage()" class="space-y-6">
    ...
    <div class="grid grid-cols-2 gap-4 sm:grid-cols-4">
        {{-- Total Students --}}
        <div class="relative overflow-hidden rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-900">
            <div class="absolute right-0 top-0 h-20 w-20 translate-x-4 -translate-y-4 rounded-full bg-[#8B0000]/8"></div>
            <div class="relative">
                <div class="mb-3 inline-flex h-10 w-10 items-center justify-center rounded-xl bg-[#8B0000]/10">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-[#8B0000]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"/>
                    </svg>
                </div>
                <p class="text-2xl font-extrabold text-slate-900 dark:text-white">{{ number_format($totalStudents) }}</p>
                <p class="mt-0.5 text-xs font-medium text-slate-500 dark:text-slate-400">Active Students</p>
            </div>
        </div>
        {{-- By Program --}}
        <div class="relative overflow-hidden rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-900">
            <div class="absolute right-0 top-0 h-20 w-20 translate-x-4 -translate-y-4 rounded-full bg-blue-500/8"></div>
            <div class="relative">
                <div class="mb-3 inline-flex h-10 w-10 items-center justify-center rounded-xl bg-blue-100 dark:bg-blue-900/30">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-blue-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"/>
                    </svg>
                </div>
                <p class="text-2xl font-extrabold text-slate-900 dark:text-white">{{ $byProgram->count() }}</p>
                <p class="mt-0.5 text-xs font-medium text-slate-500 dark:text-slate-400">Programs</p>
            </div>
        </div>
        {{-- By Department --}}
        <div class="relative overflow-hidden rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-900">
            <div class="absolute right-0 top-0 h-20 w-20 translate-x-4 -translate-y-4 rounded-full bg-purple-500/8"></div>
            <div class="relative">
                <div class="mb-3 inline-flex h-10 w-10 items-center justify-center rounded-xl bg-purple-100 dark:bg-purple-900/30">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-purple-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"/>
                    </svg>
                </div>
                <p class="text-2xl font-extrabold text-slate-900 dark:text-white">{{ $byDepartment->count() }}</p>
                <p class="mt-0.5 text-xs font-medium text-slate-500 dark:text-slate-400">Departments</p>
            </div>
        </div>
        {{-- By Semester --}}
        <div class="relative overflow-hidden rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-900">
            <div class="absolute right-0 top-0 h-20 w-20 translate-x-4 -translate-y-4 rounded-full bg-amber-500/8"></div>
            <div class="relative">
                <div class="mb-3 inline-flex h-10 w-10 items-center justify-center rounded-xl bg-amber-100 dark:bg-amber-900/30">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-amber-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"/>
                    </svg>
                </div>
                <p class="text-2xl font-extrabold text-slate-900 dark:text-white">{{ $bySemester->count() }}</p>
                <p class="mt-0.5 text-xs font-medium text-slate-500 dark:text-slate-400">Semesters Active</p>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 gap-4 lg:grid-cols-3">
        ...
        <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-900">
            <h2 class="mb-4 text-sm font-bold uppercase tracking-wide text-slate-500 dark:text-slate-400">By Program</h2>
            @forelse($byProgram->sortByDesc('total') as $row)
            @php $pct = $totalStudents > 0 ? round(($row->total / $totalStudents) * 100) : 0; @endphp
            <div class="mb-3">
                <div class="mb-1 flex justify-between text-xs font-medium text-slate-700 dark:text-slate-300">
                    <span class="truncate pr-2">{{ $row->program?->name ?? 'Unknown' }}</span>
                    <span class="whitespace-nowrap font-bold text-[#8B0000]">{{ $row->total }} <span class="font-normal text-slate-400">({{ $pct }}%)</span></span>
                </div>
                <div class="h-2 w-full overflow-hidden rounded-full bg-slate-100 dark:bg-slate-800">
                    <div class="h-2 rounded-full bg-[#8B0000] transition-all duration-700" style="width: {{ $pct }}%"></div>
                </div>
            </div>
            @empty
        ...
        <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-900">
            <h2 class="mb-4 text-sm font-bold uppercase tracking-wide text-slate-500 dark:text-slate-400">By Department</h2>
            @forelse($byDepartment->sortByDesc('total') as $row)
            @php $pct = $totalStudents > 0 ? round(($row->total / $totalStudents) * 100) : 0; @endphp
            <div class="mb-3">
                <div class="mb-1 flex justify-between text-xs font-medium text-slate-700 dark:text-slate-300">
                    <span class="truncate pr-2">{{ $row->department?->name ?? 'Unknown' }}</span>
                    <span class="whitespace-nowrap font-bold text-blue-600">{{ $row->total }} <span class="font-normal text-slate-400">({{ $pct }}%)</span></span>
                </div>
                <div class="h-2 w-full overflow-hidden rounded-full bg-slate-100 dark:bg-slate-800">
                    <div class="h-2 rounded-full bg-blue-500 transition-all duration-700" style="width: {{ $pct }}%"></div>
                </div>
            </div>
            @empty
            <p class="text-sm text-slate-400">No data</p>
        ...
        {{-- Semester Breakdown --}}
        <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-900">
            <h2 class="mb-4 text-sm font-bold uppercase tracking-wide text-slate-500 dark:text-slate-400">By Semester</h2>
            @forelse($bySemester->sortByDesc('total') as $row)
            @php $pct = $totalStudents > 0 ? round(($row->total / $totalStudents) * 100) : 0; @endphp
            <div class="mb-3">
                <div class="mb-1 flex justify-between text-xs font-medium text-slate-700 dark:text-slate-300">
                    <span>{{ $row->current_semester ? 'Semester ' . $row->current_semester : 'No Semester' }}</span>
                    <span class="font-bold text-purple-600">{{ $row->total }} <span class="font-normal text-slate-400">({{ $pct }}%)</span></span>
                </div>
                <div class="h-2 w-full overflow-hidden rounded-full bg-slate-100 dark:bg-slate-800">
                    <div class="h-2 rounded-full bg-purple-500 transition-all duration-700" style="width: {{ $pct }}%"></div>
                </div>
            </div>
            @empty
            <p class="text-sm text-slate-400">No data</p>
