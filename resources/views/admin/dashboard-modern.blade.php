...
<div id="principal-dashboard"
    ...
    <div class="rounded-lg border border-slate-200 bg-white px-6 py-5 shadow-sm">
        ...
        <div class="mt-3 flex flex-wrap items-center gap-2 border-t border-slate-100 pt-3">
            @php
                $dashAcademicYear = \Illuminate\Support\Facades\Cache::remember('public:site_settings', 600, fn() =>
                    \App\Models\SiteSetting::all()->pluck('value', 'key')->toArray()
                )['academic_year'] ?? null;
            @endphp
            @if($dashAcademicYear)
                <span class="inline-flex items-center gap-1.5 rounded bg-amber-50 border border-amber-200 px-2.5 py-1 text-xs font-bold text-amber-700">
