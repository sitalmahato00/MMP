...
<form method="POST" action="{{ route('admin.students.store') }}" enctype="multipart/form-data"
    ...
    <div class="mb-6 flex gap-1 rounded-xl bg-slate-100 p-1">
        ...
        <button type="button" @click="tab='parent'"
                :class="tab==='parent' ? 'bg-white shadow-sm text-slate-900 font-semibold' : 'text-slate-500 hover:text-slate-700'"
                class="flex-1 rounded-lg px-4 py-2.5 text-sm font-medium transition">Parent Account</button>
    </div>
    ...
    <div x-show="tab==='health'" x-transition>
        ...
        <div class="mt-4 flex gap-3">
            ...
            <button type="button" @click="tab='parent'" class="rounded-xl bg-[#8B0000] px-5 py-2.5 text-sm font-semibold text-white hover:bg-[#7a0000] transition">Next: Parent Account →</button>
        </div>
    </div>
