...
@endsection
            <div class="space-y-2">
                ...
                <select name="featured" class="w-full rounded-2xl border border-slate-300 bg-white px-4 py-3 text-sm text-slate-900 outline-none transition focus:border-[#8B0000] focus:ring-4 focus:ring-[#8B0000]/10">
        ...
        <div class="mt-5 flex flex-wrap items-center justify-between gap-3">
            <div class="text-sm text-slate-500">
                Showing {{ $staff->firstItem() ?? 0 }}-{{ $staff->lastItem() ?? 0 }} of {{ $staff->total() }} staff records.
            </div>
            <div class="flex flex-wrap gap-3">
                ...
                <a href="{{ route('admin.staff.index') }}" class="inline-flex items-center justify-center rounded-full border border-slate-300 bg-white px-5 py-2.5 text-sm font-semibold text-slate-700 transition hover:border-[#8B0000] hover:text-[#8B0000]">Reset</a>
            </div>
            <div x-data="{ view: localStorage.getItem('mmp_staff_view') ?? 'table', setView(v) { this.view = v; localStorage.setItem('mmp_staff_view', v); } }" class="space-y-5">
                <div class="flex flex-wrap items-start justify-between gap-4">
                    <div>
                        <h1 class="text-2xl font-black tracking-tight text-slate-900">Administrative Staff</h1>
                        <p class="mt-0.5 text-sm text-slate-500">Manage staff profiles, documents, and public visibility.</p>
                    </div>
                    <div class="flex flex-wrap items-center gap-2">
                        <a href="{{ route('admin.staff.create') }}"
                           class="inline-flex items-center gap-2 rounded-xl bg-[#8B0000] px-4 py-2 text-sm font-bold text-white shadow-sm hover:bg-[#7a0000] transition">
                ...
                <form method="POST" action="{{ route('admin.staff.import') }}" enctype="multipart/form-data" class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                    ...
                    <div class="flex flex-wrap items-end gap-3">
                        ...
                        <button type="submit" class="rounded-xl bg-[#8B0000] px-5 py-2.5 text-sm font-bold text-white hover:bg-[#7a0000] transition shadow-sm">Import Staff CSV</button>
                        <p class="text-sm text-slate-500">Upload the staff CSV schema to create or update records.</p>
                    </div>
                </form>
                ...
                <div class="grid grid-cols-2 lg:grid-cols-5 gap-4">
                    ...
                    <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                        <div class="flex items-center justify-between">
                            ...
                            @if(isset($kpi['meta']))
                                <span class="rounded-full bg-{{ $kpi['color'] }}-50 px-2 py-0.5 text-[11px] font-bold text-{{ $kpi['color'] }}-700">{{ $kpi['meta'] }}</span>
                            @else
                                <span class="rounded-full bg-{{ $kpi['color'] }}-50 px-2 py-0.5 text-[11px] font-bold text-{{ $kpi['color'] }}-700">Staff</span>
                            @endif
                        </div>
                        @if(is_numeric($kpi['value']))
                            <p class="mt-3 text-3xl font-black text-slate-900">{{ number_format($kpi['value']) }}</p>
