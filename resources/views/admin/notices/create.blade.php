@extends('layouts.app')
@php
    $defaultType = old('type', request('type', array_key_first($workspace['type_options'])));
@endphp
    ...
    <x-form-section :title="$workspace['detail_heading']">
        <x-form-row :cols="1">
            <x-form-field label="Title" name="title" :required="true">
                <x-input name="title" :required="true" :placeholder="$workspace['is_news_events'] ? 'Post title...' : 'Notice title...'" />
            </x-form-field>
                        ...
                        @foreach($departments as $department)
                            @php
                                $departmentLabel = $department->code
                                    ...
                                    : $department->name;
                            @endphp
                            <option value="{{ $department->id }}" @selected((string) old('department_id') === (string) $department->id)>{{ $departmentLabel }}</option>
                        @endforeach
                    </x-select>
                </x-form-field>
            </div>
