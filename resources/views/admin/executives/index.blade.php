...
<div class="bg-white rounded-xl border border-gray-100 shadow-sm px-4 py-3">
    <form method="GET" class="flex flex-wrap gap-3 items-center">
        <x-input name="search" value="{{ request('search') }}" placeholder="Search name or designation…" class="flex-1 min-w-[200px]"/>
        <x-select name="type">
        ...
        @if(request()->anyFilled(['search','type','status']))
            <x-btn href="{{ route('admin.executives.index') }}" variant="ghost" size="sm">Clear</x-btn>
        @endif
    </form>
</div>
        ...
        <div class="bg-white rounded-xl border border-gray-100 shadow-sm hover:shadow-md transition-shadow overflow-hidden">
            <div class="p-5">
                ...
                <div class="mt-4 flex items-center gap-2">
                    <button @click="drawerOpen = true; selectedExecutive = {{ $executive->id }}"
