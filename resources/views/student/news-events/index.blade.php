...
@section('content')
<div x-data="{
    view: localStorage.getItem('mmp_student_news_events_view') ?? 'table',
    setView(v) { this.view = v; localStorage.setItem('mmp_student_news_events_view', v); }
}" class="space-y-6">
    <div>
        <h1 class="text-2xl font-black tracking-tight text-slate-900">News & Events</h1>
        <p class="mt-0.5 text-sm text-slate-500">{{ $student->program->department->name }} - latest college news and upcoming events</p>
    </div>

    <form method="GET" action="{{ route('student.news-events.index') }}" class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
        ...
        <div class="flex gap-2 mt-3">
            ...
            @if(request()->hasAny(['search', 'type']))
                <a href="{{ route('student.news-events.index') }}" class="inline-flex items-center justify-center rounded-xl border border-slate-200 px-3 py-2.5 text-sm text-slate-500 hover:bg-slate-50 transition">Clear</a>
            @endif
        </div>
    </form>
        ...
        <div x-show="view === 'table'" x-cloak>
            @if($items->isEmpty())
                <div class="flex flex-col items-center justify-center py-20 text-center">
                    ...
                    <p class="mt-1 text-sm text-slate-500">There are no published updates for you right now.</p>
                </div>
            @else
                                ...
                                <tr class="group hover:bg-slate-50/60 transition-colors">
                                    ...
                                    <td class="px-5 py-3.5">
                                        <div class="flex items-center justify-end gap-1">
                                            <a href="{{ route('student.news-events.show', $item) }}" class="inline-flex h-8 w-8 items-center justify-center rounded-lg text-slate-400 hover:bg-blue-50 hover:text-blue-600 transition" title="View">
                    ...
                    @foreach($items as $item)
                        <div class="group relative rounded-2xl border border-slate-200 bg-white p-5 shadow-sm hover:shadow-md hover:border-slate-300 transition-all duration-150">
                            ...
                            <div class="mt-4 grid grid-cols-1 gap-2">
                                <a href="{{ route('student.news-events.show', $item) }}" class="rounded-lg bg-slate-900 py-1.5 text-center text-xs font-bold text-white hover:bg-slate-700 transition">View</a>
                            </div>
                        </div>
                    @endforeach
                </div>
