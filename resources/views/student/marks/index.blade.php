...
<div class="space-y-6">
        ...
        <div class="relative px-4 py-4 sm:px-6 sm:py-5 lg:px-8">
            <div class="flex flex-col gap-3 lg:gap-4 lg:flex-row lg:items-center lg:justify-between">
                <div>
                    ...
                    <p class="mt-1 text-sm text-slate-600">View your published assessments and marksheets</p>
                </div>
            </div>
        </div>
    </section>
    ...
    <section class="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <div class="rounded-xl border border-slate-200/80 bg-white p-4 shadow-sm">
            ...
            <div class="mt-3">
                <span class="text-2xl font-bold tracking-tight text-slate-900">{{ number_format($totalAssessments) }}</span>
                <p class="mt-0.5 text-[11px] font-medium uppercase tracking-wider text-slate-400">Published Assessments</p>
            </div>
        </div>
        ...
        <div class="rounded-xl border border-slate-200/80 bg-white p-4 shadow-sm">
            ...
            <div class="mt-3">
                <div class="flex items-baseline gap-1">
                    <span class="text-2xl font-bold tracking-tight text-slate-900">{{ number_format($averagePercentage, 1) }}</span>
                    <span class="text-sm font-medium text-slate-400">%</span>
                </div>
                <p class="mt-0.5 text-[11px] font-medium uppercase tracking-wider text-slate-400">Percentage Rate</p>
            </div>
        </div>
    ...
    <section class="rounded-xl border border-slate-200/80 bg-white p-4 shadow-sm">
        <form method="GET" class="flex flex-wrap items-end gap-4">
            <div class="min-w-[150px]">
                <label class="mb-1 block text-xs font-medium text-slate-700">Exam Type</label>
    ...
    <section class="rounded-xl border border-slate-200/80 bg-white shadow-sm">
        <div class="border-b border-slate-100 px-5 py-4">
            ...
            <p class="text-xs text-slate-500">Your published assessments with marksheet access</p>
        </div>
        ...
        @if($assessmentResults->isNotEmpty())
            <div class="border-t border-slate-100 px-5 py-4 text-xs text-slate-500">
