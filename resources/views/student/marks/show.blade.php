...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <!-- Marksheet Card -->
    <div id="marksheet" class="bg-white rounded-xl shadow-lg border-2 border-slate-200 overflow-hidden print:shadow-none print:border print:rounded-none print:mt-0">

        <!-- Header with College Info -->
        <div class="border-b-4 border-slate-900 bg-white px-4 sm:px-8 py-6">
            <!-- Result badge top-right on mobile -->
            <div class="flex justify-end mb-2 sm:hidden">
                <span class="inline-flex items-center px-3 py-1 rounded-lg text-xs font-bold {{ $allPassed ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700' }}">
            ...
            <div class="flex flex-col sm:flex-row items-center gap-4">
                ...
                <div class="flex-shrink-0">
                    <img src="{{ route('public.brand-logo') }}?v={{ logoVersion() }}"
                         alt="College Logo"
                         class="w-16 h-16 sm:w-20 sm:h-20 object-contain rounded-lg border-2 border-slate-200"
                         onerror="this.style.display='none'">
                </div>

                <!-- College Details — centered on all sizes -->
                <div class="flex-1 text-center">
                    <h1 class="text-lg sm:text-2xl font-black text-slate-900 uppercase tracking-wide leading-tight">
                        {{ \App\Models\SiteSetting::where('key', 'site_name')->value('value') ?? 'Manmohan Memorial Polytechnic' }}
                    </h1>
                    <p class="text-xs sm:text-sm text-slate-600 mt-1">{{ $student->department->name ?? 'N/A' }}</p>
                    <p class="text-xs text-slate-500 mt-0.5">{{ $student->program->name ?? 'N/A' }}</p>
                    <div class="mt-3 pt-3 border-t border-slate-200">
                        <h2 class="text-base sm:text-lg font-bold text-purple-700 uppercase">{{ $exam->name }}</h2>
                        <p class="text-xs text-slate-600 mt-1">{{ $exam->category_label }} - {{ bsDate($exam->published_at ?? $exam->created_at) }}</p>
                    </div>
                </div>

                <!-- Result Status Badge — hidden on mobile (shown above) -->
        ...
        <!-- Student Information -->
        <div class="bg-slate-50 border-b border-slate-200 px-4 sm:px-8 py-4">
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
                <div>
                    <p class="text-xs font-semibold text-slate-500 uppercase">Student Name</p>
                    <p class="text-sm font-bold text-slate-900 mt-1 break-words">{{ $student->user->name }}</p>
                </div>
                <div>
        ...
        <!-- Marks Table — scrollable on very small screens -->
        <div class="overflow-x-auto -mx-0">
            <table class="w-full text-xs sm:text-sm min-w-[480px]">
                <thead>
                    <tr class="bg-slate-900 text-white">
                        <th class="px-3 sm:px-4 py-3 text-left font-bold uppercase text-xs">Subject</th>
                        <th class="px-3 sm:px-4 py-3 text-center font-bold uppercase text-xs">Full</th>
                        <th class="px-3 sm:px-4 py-3 text-center font-bold uppercase text-xs">Pass</th>
                        <th class="px-3 sm:px-4 py-3 text-center font-bold uppercase text-xs">Obtained</th>
                        <th class="px-3 sm:px-4 py-3 text-center font-bold uppercase text-xs">Attendance</th>
                        <th class="px-3 sm:px-4 py-3 text-center font-bold uppercase text-xs">Remarks</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-200">
                    @foreach($marksData as $data)
                        ...
                        <tr class="hover:bg-slate-50 transition-colors print:hover:bg-transparent">
                            <td class="px-3 sm:px-4 py-3">
                                <div>
                                    <p class="font-semibold text-slate-900 leading-tight">{{ $mark->subject->name }}</p>
                                    <p class="text-xs text-slate-500">{{ $mark->subject->code }}</p>
                                </div>
                            </td>

                            @if($exam->category === 'monthly_assessment')
                                <td class="px-3 sm:px-4 py-3 text-center font-semibold text-slate-900">
                                    {{ number_format($mark->assessment_full_marks ?? 0, 0) }}
                                </td>
                                <td class="px-3 sm:px-4 py-3 text-center font-semibold text-slate-600">
                                    {{ number_format($mark->assessment_pass_marks ?? 0, 0) }}
                                </td>
                                <td class="px-3 sm:px-4 py-3 text-center">
                                    <span class="font-bold text-base sm:text-lg {{ $mark->is_passed ? 'text-green-600' : 'text-red-600' }}">
                            ...
                            <td class="px-3 sm:px-4 py-3 text-center">
                                @if($mark->assessment_attendance_percent !== null)
                                    <span class="text-xs sm:text-sm font-semibold {{ $mark->assessment_attendance_percent >= 80 ? 'text-green-600' : 'text-amber-600' }}">
                    ...
                    <tr class="bg-slate-100 font-bold print:bg-gray-100">
                        <td class="px-3 sm:px-4 py-4 text-right uppercase text-slate-900">Total</td>
                        <td class="px-3 sm:px-4 py-4 text-center text-slate-900">{{ number_format($totalFull, 0) }}</td>
                        <td class="px-3 sm:px-4 py-4 text-center text-slate-600">-</td>
                        <td class="px-3 sm:px-4 py-4 text-center">
                            <span class="text-base sm:text-lg {{ $allPassed ? 'text-green-600' : 'text-red-600' }}">
                                {{ number_format($totalObtained, 2) }}
                            </span>
                        </td>
                        <td class="px-3 sm:px-4 py-4 text-center text-slate-600">-</td>
                        <td class="px-3 sm:px-4 py-4 text-center">
                            <span class="inline-flex items-center px-2 sm:px-3 py-1 rounded-full text-xs sm:text-sm font-bold
        ...
        <!-- Summary Section -->
        <div class="bg-slate-50 border-t-2 border-slate-200 px-4 sm:px-8 py-6">
            <div class="grid grid-cols-3 gap-3 sm:gap-6">
                <div class="text-center">
                    <p class="text-xs font-semibold text-slate-500 uppercase">Total Marks</p>
                    <p class="text-base sm:text-2xl font-bold text-slate-900 mt-1 leading-tight">
                        {{ number_format($totalObtained, 2) }}<span class="text-xs sm:text-base font-normal"> / {{ number_format($totalFull, 0) }}</span>
                    </p>
                </div>
                <div class="text-center">
                    <p class="text-xs font-semibold text-slate-500 uppercase">Percentage</p>
                    <p class="text-base sm:text-2xl font-bold text-purple-600 mt-1">{{ $percentage }}%</p>
                </div>
                <div class="text-center">
                    <p class="text-xs font-semibold text-slate-500 uppercase">Result</p>
                    <p class="text-base sm:text-2xl font-bold {{ $allPassed ? 'text-green-600' : 'text-red-600' }} mt-1">
                        {{ $allPassed ? 'PASSED' : 'FAILED' }}
                    </p>
                </div>
            </div>
        ...
        <!-- Footer -->
        <div class="border-t border-slate-200 px-4 sm:px-8 py-6">
...
/* Mobile: ensure table scrolls inside card cleanly */
