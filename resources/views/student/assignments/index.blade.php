...
<div class="space-y-6">
    ...
    <section class="relative overflow-hidden rounded-xl border border-slate-200/80 bg-white shadow-sm">
        ...
        <div class="relative px-4 py-4 sm:px-6 sm:py-5 lg:px-8">
            <div class="flex flex-col gap-3 lg:gap-4 lg:flex-row lg:items-center lg:justify-between">
                <div>
                    <p class="text-[10px] sm:text-xs font-semibold uppercase tracking-widest text-slate-400">Academic Work</p>
                    <h1 class="mt-1 text-xl sm:text-2xl lg:text-3xl font-bold tracking-tight text-slate-900">
                        My Assignments
                    </h1>
                    <p class="mt-1 text-sm text-slate-600">View and submit your assignments</p>
                </div>
            </div>
        </div>
    </section>
    ...
    <section class="grid gap-3 sm:gap-4 grid-cols-2 lg:grid-cols-4">
        <div class="rounded-xl border border-slate-200/80 bg-white p-4 shadow-sm">
            ...
            <div class="mt-3">
                <span class="text-2xl font-bold tracking-tight text-slate-900">{{ number_format($totalAssignments) }}</span>
                <p class="mt-0.5 text-[11px] font-medium uppercase tracking-wider text-slate-400">Total Assignments</p>
            </div>
        </div>
        ...
        <div class="rounded-xl border border-slate-200/80 bg-white p-4 shadow-sm">
            ...
            <div class="mt-3">
                <span class="text-2xl font-bold tracking-tight text-slate-900">{{ number_format($pendingCount) }}</span>
        ...
        <div class="rounded-xl border border-slate-200/80 bg-white p-4 shadow-sm">
            ...
            <div class="mt-3">
                <span class="text-2xl font-bold tracking-tight text-slate-900">{{ number_format($submittedCount) }}</span>
        ...
        <div class="rounded-xl border border-slate-200/80 bg-white p-4 shadow-sm">
            ...
            <div class="mt-3">
                <span class="text-2xl font-bold tracking-tight text-slate-900">{{ number_format($gradedCount) }}</span>
                <p class="mt-0.5 text-[11px] font-medium uppercase tracking-wider text-slate-400">Graded</p>
            </div>
        </div>
    </section>
    ...
    <section class="rounded-xl border border-slate-200/80 bg-white p-4 shadow-sm">
        <form method="GET" class="flex flex-wrap items-end gap-4">
            ...
            <div class="flex gap-2">
                ...
                <a href="{{ route('student.assignments.index') }}" class="rounded-lg border border-slate-300 px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50">
                    Clear
                </a>
            </div>
        </form>
    ...
    <section class="rounded-xl border border-slate-200/80 bg-white shadow-sm">
        <div class="border-b border-slate-100 px-5 py-4">
            ...
            <p class="text-xs text-slate-500">Your course assignments and submissions</p>
        </div>
                            ...
                            <div class="flex items-start gap-3">
                                <div class="flex-1">
                                    ...
                                    @if($assignment->description)
                                        <p class="mt-1 text-xs text-slate-600 line-clamp-2">{{ $assignment->description }}</p>
                                    @endif
                                </div>
                            </div>
                        </div>
