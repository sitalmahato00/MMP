...
<div class="space-y-6">
    ...
    <section class="grid gap-3 sm:gap-4 grid-cols-2">
        <div class="rounded-xl border border-slate-200 dark:border-[#1e3a5f] bg-white dark:bg-[#132044] p-4 shadow-sm">
            ...
            <div class="mt-3">
                ...
                <p class="mt-0.5 text-[11px] font-medium uppercase tracking-wider text-slate-400 dark:text-slate-500">
                    Resources
                    @if($selectedSemester < $currentSemester)
                        <span class="text-amber-500">(Sem {{ $selectedSemester }})</span>
                    @endif
                </p>
            </div>
        </div>
    ...
    <section class="rounded-xl border border-slate-200 dark:border-[#1e3a5f] bg-white dark:bg-[#132044] p-4 shadow-sm">
        <form method="GET" class="flex flex-wrap items-end gap-4">
            ...
            <div class="flex gap-2">
                ...
                <a href="{{ route('student.downloads.index', ['semester' => $selectedSemester]) }}"
    ...
    <section class="rounded-xl border border-slate-200 dark:border-[#1e3a5f] bg-white dark:bg-[#132044] shadow-sm">
        <div class="border-b border-slate-100 dark:border-[#1e3a5f] px-5 py-4 flex items-center justify-between">
            <div>
                <h2 class="text-sm font-semibold text-slate-900 dark:text-slate-100">
                    Available Resources
                    @if($selectedSemester < $currentSemester)
                        <span class="ml-2 inline-flex items-center rounded-full bg-amber-50 dark:bg-amber-900/30 px-2 py-0.5 text-xs font-semibold text-amber-700 dark:text-amber-400">
        ...
        <div class="divide-y divide-slate-100 dark:divide-[#1e3a5f]">
                    ...
                    <div class="flex items-start gap-4">
                        @php
                            ...
                            $iconColor = match($ext) {
                                ...
                                'doc', 'docx' => 'text-blue-600 bg-blue-50 dark:bg-blue-900/30',
                                'xls', 'xlsx' => 'text-emerald-600 bg-emerald-50 dark:bg-emerald-900/30',
                                'ppt', 'pptx' => 'text-orange-600 bg-orange-50 dark:bg-orange-900/30',
                                'zip', 'rar'  => 'text-violet-600 bg-violet-50 dark:bg-violet-900/30',
                                default        => 'text-slate-600 bg-slate-50 dark:bg-slate-900/30',
                            };
                        @endphp
                        <div class="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-lg {{ $iconColor }}">
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <div class="mt-1 flex flex-wrap items-center gap-2 text-xs text-slate-500 dark:text-slate-400">
                                @if($download->subject)
                                    <span class="inline-flex items-center rounded-full bg-blue-50 dark:bg-blue-900/20 px-1.5 py-0.5 text-xs text-blue-700 dark:text-blue-300">
                                        {{ $download->subject->name }}
                                    </span>
                                @endif
                                @if($ext)
                                    <span class="inline-flex items-center rounded-full bg-slate-100 dark:bg-slate-700 px-1.5 py-0.5 text-xs font-mono text-slate-600 dark:text-slate-300 uppercase">
                                        {{ $ext }}
                                    </span>
                                @endif
                                <span>{{ bsDate($download->created_at, 'F d, Y') }}</span>
                        ...
                        <a href="{{ route('student.downloads.file', $download->id) }}"
                           class="flex-shrink-0 inline-flex items-center gap-2 rounded-lg bg-blue-600 px-3 py-2 text-sm font-medium text-white hover:bg-blue-700 transition">
            ...
            @empty
                <div class="px-5 py-12 text-center">
                    ...
                    @if($selectedSemester < $currentSemester)
                        <a href="{{ route('student.downloads.index', ['semester' => $currentSemester]) }}"
                           class="mt-2 inline-block text-xs text-blue-600 hover:underline">
                            View current semester materials →
                        </a>
                    @endif
                </div>
            @endforelse
        ...
        @if($downloads->hasPages())
            <div class="border-t border-slate-100 dark:border-[#1e3a5f] px-5 py-4">
                {{ $downloads->appends(['semester' => $selectedSemester, 'subject_id' => request('subject_id'), 'search' => request('search')])->links() }}
            </div>
        @endif
    </section>
</div>
