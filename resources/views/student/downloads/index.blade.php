...
<div class="space-y-6">
    ...
    <section class="relative overflow-hidden rounded-xl border border-slate-200/80 bg-white shadow-sm">
        ...
        <div class="relative px-4 py-4 sm:px-6 sm:py-5 lg:px-8">
            <div class="flex flex-col gap-3 lg:gap-4 lg:flex-row lg:items-center lg:justify-between">
                <div>
                    <p class="text-[10px] sm:text-xs font-semibold uppercase tracking-widest text-slate-400">Learning Resources</p>
                    <h1 class="mt-1 text-xl sm:text-2xl lg:text-3xl font-bold tracking-tight text-slate-900">
                        Study Materials
                    </h1>
                    <p class="mt-1 text-sm text-slate-600">Access course materials and resources</p>
                </div>
            </div>
        </div>
    </section>
    ...
    <section class="grid gap-3 sm:gap-4 grid-cols-2">
        <div class="rounded-xl border border-slate-200/80 bg-white p-4 shadow-sm">
            ...
            <div class="mt-3">
                <span class="text-2xl font-bold tracking-tight text-slate-900">{{ number_format($totalDownloads) }}</span>
                <p class="mt-0.5 text-[11px] font-medium uppercase tracking-wider text-slate-400">Total Resources</p>
            </div>
        </div>
        ...
        <div class="rounded-xl border border-slate-200/80 bg-white p-4 shadow-sm">
            ...
            <div class="mt-3">
                <span class="text-2xl font-bold tracking-tight text-slate-900">{{ number_format($subjectCount) }}</span>
    ...
    <section class="rounded-xl border border-slate-200/80 bg-white p-4 shadow-sm">
        <form method="GET" class="flex flex-wrap items-end gap-4">
            ...
            <div class="flex gap-2">
                ...
                <a href="{{ route('student.downloads.index') }}" class="rounded-lg border border-slate-300 px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50">
                    Clear
                </a>
            </div>
        </form>
    ...
    <section class="rounded-xl border border-slate-200/80 bg-white shadow-sm">
        <div class="border-b border-slate-100 px-5 py-4">
            <h2 class="text-sm font-semibold text-slate-900">Available Resources</h2>
            <p class="text-xs text-slate-500">Study materials uploaded by your teachers</p>
        </div>
        ...
        <div class="divide-y divide-slate-100">
                ...
                <div class="px-5 py-4 hover:bg-slate-50">
                    <div class="flex items-start gap-4">
                        ...
                        <div class="flex-1 min-w-0">
                            <h3 class="text-sm font-medium text-slate-900">{{ $download->title }}</h3>
                            ...
                            @if($download->description)
                                <p class="mt-1 text-xs text-slate-600 line-clamp-2">{{ $download->description }}</p>
                            @endif
                        </div>
            ...
            @empty
                <div class="px-5 py-12 text-center">
                    ...
                    <p class="mt-2 text-sm text-slate-500">No study materials available yet</p>
                </div>
            @endforelse
        </div>
