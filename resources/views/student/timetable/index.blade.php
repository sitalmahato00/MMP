...
<div class="space-y-6">
    ...
    <section class="relative overflow-hidden rounded-xl border border-slate-200/80 bg-white shadow-sm">
        ...
        <div class="relative px-4 py-4 sm:px-6 sm:py-5 lg:px-8">
            <div class="flex flex-col gap-3 lg:gap-4 lg:flex-row lg:items-center lg:justify-between">
                <div>
                    <p class="text-[10px] sm:text-xs font-semibold uppercase tracking-widest text-slate-400">Class Schedule</p>
                    <h1 class="mt-1 text-xl sm:text-2xl lg:text-3xl font-bold tracking-tight text-slate-900">
                        My Timetable
                    </h1>
                    <p class="mt-1 text-sm text-slate-600">Your weekly class schedule</p>
                </div>
            </div>
        </div>
    </section>
    ...
    @if($timetable)
        ...
        <section class="grid gap-3 sm:gap-4 grid-cols-2 lg:grid-cols-4">
            ...
            <div class="rounded-xl border border-slate-200/80 bg-white p-4 shadow-sm">
                ...
                <div class="mt-3">
                    <span class="text-2xl font-bold tracking-tight text-slate-900">{{ number_format($totalPresent) }}</span>
            ...
            <div class="rounded-xl border border-slate-200/80 bg-white p-4 shadow-sm">
                ...
                <div class="mt-3">
                    <span class="text-2xl font-bold tracking-tight text-slate-900">{{ number_format($totalAbsent) }}</span>
            ...
            <div class="rounded-xl border border-slate-200/80 bg-white p-4 shadow-sm">
                ...
                <div class="mt-3">
                    <div class="flex items-baseline gap-1">
                        <span class="text-2xl font-bold tracking-tight text-slate-900">{{ number_format($overallRate, 1) }}</span>
        ...
        {{-- Timetable Info --}}
        <section class="rounded-xl border border-slate-200/80 bg-white shadow-sm p-4">
            <div class="flex flex-wrap items-center gap-4 text-sm">
                <div>
                    <span class="text-slate-600">Program:</span>
                    <span class="ml-1 font-medium text-slate-900">{{ $student->program->name }}</span>
                </div>
                <span class="text-slate-300">•</span>
                <div>
                    <span class="text-slate-600">Semester:</span>
                    <span class="ml-1 font-medium text-slate-900">{{ $student->current_semester }}</span>
                </div>
                <span class="text-slate-300">•</span>
                <div>
                    <span class="text-slate-600">Academic Year:</span>
                    <span class="ml-1 font-medium text-slate-900">{{ $timetable->academicSession->name ?? 'N/A' }}</span>
                </div>
                <span class="text-slate-300">•</span>
                <div>
                    <span class="text-slate-600">Effective From:</span>
                    <span class="ml-1 font-medium text-slate-900">{{ bsDate($timetable->effective_from, 'F d, Y') }}</span>
                </div>
            </div>
        </section>
        ...
        <section class="rounded-xl border border-slate-200/80 bg-white shadow-sm">
            <div class="border-b border-slate-100 px-5 py-4">
                ...
                <p class="text-xs text-slate-500">Your attendance rate for each subject this semester</p>
            </div>
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @foreach($subjectAttendance as $data)
                            <tr class="hover:bg-slate-50">
                                ...
                                <td class="px-5 py-4">
                                    @if($data['last_class'])
                                        <div class="text-sm text-slate-900">{{ bsDate($data['last_class'], 'F d') }}</div>
                                        <div class="text-xs text-slate-500">{{ bsDate($data['last_class'], 'l') }}</div>
                                    @else
    ...
    @else
        {{-- No Timetable --}}
        <section class="rounded-xl border border-slate-200/80 bg-white shadow-sm p-12">
            <div class="text-center">
                <svg class="mx-auto h-16 w-16 text-slate-300" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/>
                </svg>
                <h3 class="mt-4 text-lg font-medium text-slate-900">No Timetable Available</h3>
