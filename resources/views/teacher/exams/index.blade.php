...
<div class="space-y-6">
        ...
        <div class="relative px-4 py-4 sm:px-6 sm:py-5 lg:px-8">
            <div class="flex flex-col gap-3 lg:gap-4 lg:flex-row lg:items-center lg:justify-between">
                <div>
                    ...
                    <p class="mt-1 text-sm text-slate-600">Fill marks for your assigned subjects</p>
                </div>
            </div>
        </div>
    </section>
    ...
    {{-- Filters --}}
    <x-search-filter action="{{ route('teacher.exams.index') }}">
        <div>
            <label class="block text-xs font-semibold text-slate-700 mb-1.5">Search</label>
            <x-input type="text" name="search" value="{{ request('search') }}" placeholder="Exam name..." />
        </div>
        <div>
            ...
            <table class="w-full">
                <thead class="bg-slate-50 text-xs font-medium text-slate-500 uppercase tracking-wider">
                    <tr>
                        ...
                        <th class="px-5 py-3 text-center">My Progress</th>
                    ...
                    @forelse($exams as $exam)
                        <tr class="hover:bg-slate-50">
                            ...
                            <td class="px-5 py-4">
                                ...
                                <div class="text-xs text-slate-500">{{ $exam->category_label ?? str_replace('_', ' ', ucwords($exam->category)) }}</div>
                            </td>
                            <td class="px-5 py-4">
                            ...
                            <td class="px-5 py-4 text-center">
                                @php
                                    ...
                                    $progressColor = $progressColors[$exam->teacher_marks_status] ?? 'bg-slate-50 text-slate-500';
                                    
                                    $progressLabels = [
                                        'not_filled' => 'Not Started',
                                        'partially_filled' => 'In Progress',
                                        'completed' => 'Completed',
                                        'not_applicable' => 'N/A',
                                    ];
                                    $progressLabel = $progressLabels[$exam->teacher_marks_status] ?? 'Unknown';
                                @endphp
                                <div class="flex flex-col items-center gap-1">
                                    <span class="inline-flex items-center rounded-full px-2 py-1 text-xs font-medium {{ $progressColor }}">
                                        {{ $progressLabel }}
                                    </span>
                                    @if($exam->teacher_marks_status !== 'not_applicable')
                                        <div class="text-xs text-slate-500">
                                            {{ $exam->teacher_marks_count }}/{{ $exam->teacher_total_subjects }} subjects
                                        </div>
                                    @endif
                                </div>
                            </td>
                            <td class="px-5 py-4">
                                <div class="flex items-center gap-1 flex-wrap">
                                    {{-- Fill Button --}}
                                    @if($exam->teacher_marks_status !== 'not_applicable')
                                        <a href="{{ route('teacher.exams.fill-marks', ['exam_id' => $exam->id]) }}" 
                                           ...
                                           title="Fill Marks">
                                            ...
                                            {{ $exam->teacher_marks_status === 'completed' ? 'Edit' : 'Fill' }}
                                        </a>
                                    @else
                    ...
                    @empty
                        <tr>
                            <td colspan="8" class="px-5 py-12 text-center">
                                ...
                                <p class="mt-2 text-sm text-slate-500">No exams found for your assigned subjects</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
