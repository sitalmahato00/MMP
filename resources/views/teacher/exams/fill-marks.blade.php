...
<div class="space-y-6">
    ...
    <x-page-header 
        ...
        :subtitle="$exam->name . ' • ' . ($exam->category_label ?? str_replace('_', ' ', ucwords($exam->category)))"
        back="{{ route('teacher.exams.fill-marks', ['exam_id' => $exam->id]) }}" />
    ...
    {{-- Quick Actions --}}
    <x-card x-data="marksForm()">
        <div class="flex flex-wrap items-center gap-3">
            <x-btn type="button" variant="secondary" @click="markAllPresent()">
                <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/>
                </svg>
                Mark All Present
            </x-btn>
            <x-btn type="button" variant="secondary" @click="markAllAbsent()">
                <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    {{-- Marks Table Form --}}
    <form method="POST" action="{{ route('teacher.exams.save-marks') }}" x-data="marksForm()">
        @csrf
        ...
        <div class="flex items-center gap-3 pb-6">
            ...
            <x-btn href="{{ route('teacher.exams.fill-marks', ['exam_id' => $exam->id]) }}" variant="secondary">
                Back to Selection
            </x-btn>
        </div>
    </form>
