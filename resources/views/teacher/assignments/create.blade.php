...
<div class="space-y-6">
    ...
    {{-- Form --}}
    <form action="{{ route('teacher.assignments.store') }}" method="POST" enctype="multipart/form-data" class="rounded-xl border border-slate-200/80 bg-white p-4 sm:p-6 shadow-sm space-y-6">
        @csrf
        ...
        <div>
            ...
            <input type="text" name="title" required value="{{ old('title') }}" placeholder="Assignment title..." 
        ...
        <div>
            ...
            <textarea name="description" rows="4" placeholder="Assignment description..." 
                class="w-full rounded-lg border border-slate-300 px-4 py-2 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500 @error('description') border-rose-500 @enderror">{{ old('description') }}</textarea>
            @error('description')
                <p class="mt-1 text-xs text-rose-600">{{ $message }}</p>
            @enderror
        </div>
        ...
        {{-- Attachment --}}
        <div>
            <label class="block text-sm font-semibold text-slate-900 mb-2">Attachment</label>
            <div class="relative">
        ...
        <div class="flex gap-3 border-t border-slate-100 pt-6">
            ...
            <a href="{{ route('teacher.assignments.index') }}" class="inline-flex items-center gap-2 rounded-lg border border-slate-300 px-6 py-2 text-sm font-semibold text-slate-700 transition hover:bg-slate-50">
                Cancel
            </a>
        </div>
    </form>
