    ...
    @if($notice->attachment || $notice->attachments->isNotEmpty())
        <x-card>
            <div class="mb-4">
                ...
                <p class="text-xs text-slate-500">{{ ($notice->attachment ? 1 : 0) + $notice->attachments->count() }} file{{ ((($notice->attachment ? 1 : 0) + $notice->attachments->count()) > 1) ? 's' : '' }} attached</p>
            </div>
                ...
                @if($notice->attachment)
                    @php
                        ...
                        $fileSize = Storage::disk('public')->exists($notice->attachment)
                            ...
                            : null;
                    @endphp
                    <a href="{{ asset('storage/' . ltrim($notice->attachment, '/')) }}" target="_blank" 
                       class="flex items-center gap-3 rounded-lg border border-slate-200 p-3 transition hover:bg-slate-50 hover:border-slate-300">
                        ...
                        <div class="flex-1 min-w-0">
                            <p class="text-sm font-semibold text-slate-900 truncate">{{ $fileName }}</p>
                            @if($fileSize)
