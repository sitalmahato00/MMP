    ...
    <div class="flex flex-col lg:flex-row gap-6" x-data="{ activeTab: 'profile', hasChanges: false }">
        ...
        <main class="min-w-0 flex-1">
            ...
            <div x-show="activeTab === 'profile'" x-cloak class="space-y-6">
                <form action="{{ route('parent.settings.profile.update') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                    @csrf
                    @method('PATCH')
                                    ...
                                    <div class="mt-4 text-center">
                                        ...
                                        <div class="mt-4 space-y-2">
                                            <div class="flex items-center justify-between rounded-lg bg-white px-3 py-2 text-xs">
                                                ...
                                                <span class="font-semibold text-slate-900">Parent</span>
                                            </div>
                                            @if($user->parentProfile && $user->parentProfile->children->count() > 0)
                                            <div class="flex items-center justify-between rounded-lg bg-white px-3 py-2 text-xs">
                                                <span class="text-slate-500">Children</span>
                                                <span class="font-semibold text-slate-900">{{ $user->parentProfile->children->count() }}</span>
                                            </div>
                                            @endif
                                        </div>
                                    </div>
            ...
            <div x-show="activeTab === 'security'" x-cloak class="space-y-6">
                ...
                <div class="rounded-xl border border-slate-200/80 bg-white shadow-sm">
                    ...
                    <form action="{{ route('parent.settings.password.update') }}" method="POST" class="p-6">
                        @csrf
                        @method('PATCH')
                ...
                <div class="rounded-xl border border-slate-200/80 bg-white shadow-sm">
                    ...
                    <form action="{{ route('parent.settings.two-factor.update') }}" method="POST" class="p-6">
                        @csrf
                        @method('PATCH')
                ...
                <div class="rounded-xl border border-slate-200/80 bg-white shadow-sm">
                    ...
                    <div class="border-t border-slate-100 bg-slate-50 px-6 py-4">
                        <form action="{{ route('parent.settings.logout-all') }}" method="POST" x-data @submit.prevent="
            ...
            <div x-show="activeTab === 'notifications'" x-cloak class="space-y-6">
                <form action="{{ route('parent.settings.notifications.update') }}" method="POST">
                    @csrf
                    @method('PATCH')
                    ...
                    <div class="rounded-xl border border-slate-200/80 bg-white shadow-sm">
                        ...
                        <div class="divide-y divide-slate-100 p-6">
                            <div class="flex items-center justify-between py-4 first:pt-0 last:pb-0">
                                <div>
                                    <p class="text-sm font-medium text-slate-900">Child Progress Alerts</p>
                                    <p class="mt-0.5 text-xs text-slate-500">Updates about your children's academic progress</p>
                                </div>
                                <label class="relative inline-flex cursor-pointer items-center">
                                    <input type="checkbox" name="email_child_alerts" value="1" checked class="peer sr-only">
                            ...
                            <div class="flex items-center justify-between py-4">
                                <div>
                                    ...
                                    <p class="mt-0.5 text-xs text-slate-500">When your child is absent or late</p>
                                </div>
                                <label class="relative inline-flex cursor-pointer items-center">
                                    <input type="checkbox" name="email_attendance_alerts" value="1" checked class="peer sr-only">
                                    <div class="peer h-6 w-11 rounded-full bg-slate-200 after:absolute after:left-[2px] after:top-[2px] after:h-5 after:w-5 after:rounded-full after:border after:border-slate-300 after:bg-white after:transition-all after:content-[''] peer-checked:bg-blue-600 peer-checked:after:translate-x-full peer-checked:after:border-white peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-blue-300"></div>
                                </label>
                            </div>
            ...
            <div x-show="activeTab === 'danger'" x-cloak class="space-y-6">
                ...
                <div class="overflow-hidden rounded-xl border-2 border-red-200 bg-white shadow-sm">
                    ...
                    <div class="p-6">
                        ...
                        <form action="{{ route('parent.settings.reset-dashboard') }}" method="POST" class="mt-4" x-data @submit.prevent="
                ...
                <div class="overflow-hidden rounded-xl border-2 border-red-200 bg-white shadow-sm">
                    ...
                    <div class="p-6">
                        ...
                        <form action="{{ route('parent.settings.clear-preferences') }}" method="POST" class="mt-4" x-data @submit.prevent="
