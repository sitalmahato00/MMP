                ...
                <div class="p-8 md:p-10 space-y-5">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                        <div class="rounded-2xl border border-white/10 bg-white/10 p-4">
                            ...
                            <div class="text-red-50/80 mt-1">Submit once and land on the live CTEVT result page.</div>
                        </div>
                        <div class="rounded-2xl border border-white/10 bg-white/10 p-4">
                        ...
                        <div class="rounded-2xl border border-white/10 bg-white/10 p-4">
                            ...
                            <div class="text-red-50/80 mt-1">Examination Year, Level, Symbol Number, Date of Birth (B.S.).</div>
                        </div>
                        <div class="rounded-2xl border border-white/10 bg-white/10 p-4">
                            <div class="font-semibold text-white">Official URL</div>
                            <div class="text-red-50/80 mt-1 break-all">{{ $officialResultUrl }}</div>
                        </div>
                    </div>
                </div>
        ...
        <div class="xl:col-span-3 order-2 xl:order-none">
            ...
            <div class="bg-white border border-gray-200 border-t-0 shadow-sm overflow-hidden">
                ...
                <div class="bg-gradient-to-b from-white to-red-50/20 p-5 md:p-6 space-y-4">
                    @if($errors->any())
                        <div class="rounded-2xl border border-red-100 bg-red-50/80 px-4 py-3 text-sm text-red-900">
                    ...
                    <form name="frmCheckResults" id="frmCheckResults" action="{{ route('public.result.submit') }}" method="post" target="_blank" autocomplete="off" class="space-y-4">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-[220px_minmax(0,1fr)] gap-3 md:gap-6 items-start md:items-center rounded-2xl border border-gray-100 bg-white px-4 py-4 shadow-[0_1px_2px_rgba(0,0,0,0.03)]">
                            ...
                            <div class="w-full">
                                <select name="src_year" id="src_year" required class="w-full border border-gray-200 rounded-lg px-3.5 py-2.5 text-sm text-gray-800 bg-white focus:outline-none focus:ring-2 focus:ring-[#8B0000]/25 focus:border-[#8B0000] transition-all duration-150 {{ $errors->has('src_year') ? 'border-red-400 bg-red-50' : '' }}">
                                    <option value="">-- Select --</option>
                                    @foreach(['2082', '2081', '2080', '2079', '2078', '2077'] as $year)
                                        <option value="{{ $year }}" @selected($selectedYear === $year)>{{ $year }}</option>
                                    @endforeach
                                </select>
                                @error('src_year')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-[220px_minmax(0,1fr)] gap-3 md:gap-6 items-start md:items-center rounded-2xl border border-gray-100 bg-white px-4 py-4 shadow-[0_1px_2px_rgba(0,0,0,0.03)]">
                            ...
                            <div class="w-full">
                                <select name="src_level" id="src_level" required class="w-full border border-gray-200 rounded-lg px-3.5 py-2.5 text-sm text-gray-800 bg-white focus:outline-none focus:ring-2 focus:ring-[#8B0000]/25 focus:border-[#8B0000] transition-all duration-150 {{ $errors->has('src_level') ? 'border-red-400 bg-red-50' : '' }}">
                                    <option value="">-- Select --</option>
                                    <option value="2" @selected($selectedLevel === '2')>Pre-diploma</option>
                                    <option value="3" @selected($selectedLevel === '3')>Diploma/PCL</option>
                                </select>
                                @error('src_level')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-[220px_minmax(0,1fr)] gap-3 md:gap-6 items-start md:items-center rounded-2xl border border-gray-100 bg-white px-4 py-4 shadow-[0_1px_2px_rgba(0,0,0,0.03)]">
                            ...
                            <div class="w-full">
                                <input type="text" name="exam_symbol_number" id="exam_symbol_number" value="{{ $symbolValue }}" placeholder="e.g. 1000234" required class="w-full border border-gray-200 rounded-lg px-3.5 py-2.5 text-sm text-gray-800 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#8B0000]/25 focus:border-[#8B0000] transition-all duration-150 {{ $errors->has('exam_symbol_number') ? 'border-red-400 bg-red-50' : '' }}">
                                @error('exam_symbol_number')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror
                            </div>
                        </div>
                        ...
                        <div class="pt-2 flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center sm:justify-start">
                            <button type="submit" name="submit" value="Search" class="inline-flex w-full sm:w-auto items-center justify-center gap-2 rounded-lg bg-[#8B0000] px-6 py-3 text-white font-semibold shadow-sm hover:bg-[#6f0000] transition-colors">
                                Search
                            </button>
                        </div>
                    </form>
