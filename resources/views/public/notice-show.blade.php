...
@php
    ...
    $images           = $notice->attachments->filter(fn($a) => $a->is_image);
    $videos           = $notice->attachments->filter(fn($a) => in_array($a->file_type, ['mp4','webm','mov','avi']));
    $otherAttachments = $notice->attachments->filter(fn($a) => !$a->is_image && !in_array($a->file_type, ['mp4','webm','mov','avi']));
    $hasMedia         = $images->count() > 0 || $videos->count() > 0;
    $quickLinks = [
            ...
            <div class="bg-white rounded-lg shadow-md overflow-hidden">
                ...
                <div class="px-6 py-4 border-b border-gray-200">
                    <div class="flex items-center gap-2 mb-3 flex-wrap">
                        <span class="text-xs font-bold text-blue-700 bg-blue-50 px-2 py-1 rounded border border-blue-100 uppercase">
                            {{ $notice->type }}
                        </span>
                        ...
                        <div class="space-y-3">
                            ...
                            @foreach($otherAttachments as $att)
                                ...
                                <div class="flex items-center gap-3 p-3 bg-white rounded-lg border border-gray-200 hover:border-blue-300 transition-colors">
                                    ...
                                    <div class="flex-1 min-w-0">
                                        <p class="text-sm font-medium text-gray-900 truncate">{{ $att->file_name }}</p>
                                        <p class="text-xs text-gray-500">
                                            @if($att->file_size)
                                                {{ number_format($att->file_size / 1024, 1) }} KB
                                            @endif
                                            @if($att->file_type)
                                                &bull; {{ strtoupper($att->file_type) }}
                                            @endif
                                        </p>
                                    </div>
            ...
            @if($relatedNotices->count() > 0)
                <div>
                    ...
                    <div class="bg-white border border-gray-200 border-t-0 rounded-b-lg shadow-md">
                        @foreach($relatedNotices->take(5) as $rel)
                            @if($rel->id !== $notice->id)
                                @php $relDate = $rel->published_at ?? $rel->created_at; @endphp
                                <a href="{{ route('public.notice.show', $rel->slug) }}"
                                   class="flex items-start gap-3 px-4 py-3 border-b border-gray-100 last:border-0 text-sm text-gray-700 hover:bg-blue-50 hover:text-blue-800 transition-colors">
                                    <div class="flex-shrink-0 w-8 h-10 text-white flex flex-col items-center justify-center rounded text-center text-xs" style="background-color: #003D82;">
                                        <span class="font-bold leading-none">{{ bsDate($relDate, 'd') }}</span>
                                        <span class="text-[8px] uppercase leading-none">{{ bsDate($relDate, 'M') }}</span>
                                    </div>
                                    <div class="flex-1 min-w-0">
                                        <p class="font-medium leading-snug">{{ $rel->title }}</p>
                                        <span class="text-xs font-bold text-blue-700 bg-blue-50 px-1.5 py-0.5 rounded uppercase mt-1 inline-block">{{ $rel->type }}</span>
                                    </div>
                                </a>
                            @endif
                        @endforeach
            ...
            <div>
                ...
                <div class="bg-white border border-gray-200 border-t-0 rounded-b-lg shadow-md">
                    @foreach($quickLinks as $link)
                        <a href="{{ $link['href'] }}"
                           class="flex items-center gap-3 px-4 py-3 border-b border-gray-100 last:border-0 text-sm text-gray-700 hover:bg-blue-50 hover:text-blue-800 transition-colors">
