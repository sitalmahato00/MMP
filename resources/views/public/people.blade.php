...
<div class="w-full px-4 md:px-8 xl:px-16 2xl:px-24 mx-auto py-8">
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div class="lg:col-span-2 space-y-6">
            <div class="section-header flex items-center justify-between pr-3" style="background-color: #003D82;">
            ...
            <div class="bg-white border border-gray-200 p-4 shadow-sm space-y-3" x-data="{ deptMenuOpen: false }">
                ...
                <div class="relative max-w-xl">
                    <button type="button"
                        ...
                        class="flex w-full items-center justify-between gap-3 rounded-md border border-gray-300 bg-white px-4 py-3 text-left text-sm font-semibold text-gray-700 shadow-sm transition hover:border-[#003D82] hover:text-[#003D82]">
                    ...
                    <div x-show="deptMenuOpen" x-cloak @click.outside="deptMenuOpen = false"
                        ...
                        <a href="{{ route('public.people') }}"
                            class="block px-4 py-3 text-sm text-gray-700 hover:bg-blue-50 hover:text-[#003D82] transition-colors {{ $selectedDepartmentSlug === '' ? 'bg-blue-50 text-[#003D82]' : '' }}">
                            All Departments
                        </a>
                        @foreach($departmentOptions as $department)
                            <a href="{{ route('public.people', ['department' => $department->slug]) }}"
                                class="block px-4 py-3 text-sm text-gray-700 hover:bg-blue-50 hover:text-[#003D82] transition-colors {{ $selectedDepartmentSlug === $department->slug ? 'bg-blue-50 text-[#003D82]' : '' }}">
                                {{ $department->name }}
                            </a>
                        @endforeach
                    </div>
                                            ...
                                            <div>
                                                ...
                                                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-5">
                                                    @foreach($category['members'] as $member)
                                                        <div class="group text-center p-4 border border-gray-100 rounded hover:border-[#003D82]/30 hover:shadow-md transition-all">
                                                            <div class="w-24 h-24 mx-auto mb-3 rounded-full overflow-hidden border-4 border-gray-100 group-hover:border-[#003D82]/20 transition-colors bg-gray-200 shadow-sm">
                                                            ...
                                                            @if($member->department)
                                                                ...
                                                                @else
                                                                    <span class="text-[10px] text-blue-700 bg-blue-50 px-2 py-0.5 rounded inline-block mt-2 border border-blue-100">{{ $member->department }}</span>
                                                                @endif
                                                            @endif
                                                            @if(!empty($member->profile_url))
                                                                <a href="{{ $member->profile_url }}" class="mt-4 inline-flex items-center justify-center rounded-full bg-[#003D82] px-4 py-2 text-[11px] font-bold uppercase tracking-wide text-white transition-colors hover:bg-[#001F4D]">
                                                                    View Profile
                                                                </a>
                                                            @endif
                                                        </div>
        ...
        <div class="space-y-6">
            ...
            @if($departments->count() > 0)
                <div>
                    <div class="section-header" style="background-color: #003D82;">🏛️ Departments</div>
                    <div class="bg-white border border-gray-200 border-t-0 rounded-b-lg shadow-md">
                        @foreach($departmentOptions as $department)
                            <a href="{{ route('public.department.show', $department->slug) }}" class="flex items-center gap-3 px-4 py-3 border-b border-gray-100 last:border-0 text-sm text-gray-700 hover:bg-blue-50 hover:text-blue-800 transition-colors">
                                <span class="text-blue-600">›</span>{{ $department->name }}
                            </a>
                        @endforeach
                    </div>
                </div>
