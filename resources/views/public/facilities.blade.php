...
<div class="bg-[#f9f9f9] border-t border-gray-100 min-h-screen">
    <div class="w-full px-4 md:px-8 xl:px-16 2xl:px-24 mx-auto py-12">
        <div class="text-center mb-8">
            <h1 class="text-3xl font-bold font-serif text-[#003D82]">Campus Facilities & Resources</h1>
        ...
        <div class="mb-8 flex justify-center">
            <div class="inline-flex flex-wrap items-center gap-2 bg-white rounded-xl border border-gray-200 p-2 shadow-sm">
                <a href="{{ route('public.facilities') }}" 
                   class="px-4 py-2 rounded-lg text-sm font-semibold transition-colors {{ !request('department') ? 'bg-[#003D82] text-white' : 'text-gray-600 hover:bg-gray-100' }}">
                    All Departments
                </a>
                @foreach($departments as $dept)
                    <a href="{{ route('public.facilities', ['department' => $dept->slug]) }}" 
                       class="px-4 py-2 rounded-lg text-sm font-semibold transition-colors {{ request('department') === $dept->slug ? 'bg-[#003D82] text-white' : 'text-gray-600 hover:bg-gray-100' }}">
                        {{ $dept->name }}
                    </a>
                @endforeach
            </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                    @foreach($items as $facility)
                        <div class="bg-white rounded shadow-sm border border-gray-200 overflow-hidden group">
                            ...
                            <div class="p-5">
                                <h3 class="font-bold text-gray-900 text-lg mb-1 group-hover:text-[#003D82] transition-colors">{{ $facility->name }}</h3>
                                ...
                                <div class="text-xs text-gray-500 mb-3 flex items-center gap-1">
                                    <svg class="w-3 h-3 text-[#003D82] flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
                                    {{ $facility->location }}
