...
<div class="w-full px-4 md:px-8 xl:px-16 2xl:px-24 mx-auto py-8">
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div class="lg:col-span-2">
            ...
            <div class="bg-white border border-gray-200 border-t-0 mb-6">
                @forelse($notices as $notice)
                    <div class="group flex items-start gap-4 px-5 py-4 border-b border-gray-100 last:border-0 hover:bg-red-50 transition-colors">
                        <div class="flex-shrink-0 w-12 h-12 text-white flex flex-col items-center justify-center rounded" style="background-color: #8B0000;">
                            <span class="text-[9px] font-bold uppercase">{{ $notice->published_at->format('M') }}</span>
                            <span class="text-xl font-black leading-tight">{{ $notice->published_at->format('d') }}</span>
                            <span class="text-[9px]">{{ $notice->published_at->format('Y') }}</span>
                        </div>
                        <div class="flex-1 min-w-0">
                            ...
                            <div class="flex items-center gap-2 mt-2 flex-wrap">
                                ...
                                <span class="text-xs text-gray-400">{{ $notice->published_at->format('F d, Y') }}</span>
