...
@section('content')
<x-page-header title="Create Content Page" subtitle="Create a new department web page."
               back="{{ route('hod.content.index') }}"/>

<form method="POST" action="{{ route('hod.content.store') }}" enctype="multipart/form-data"
        ...
        <x-form-row>
            ...
            <x-form-field label="Featured Image" name="featured_image">
                <x-file-input name="featured_image" accept="image/*" 
                             label="Upload featured image (max 2 MB)"/>
            </x-form-field>
        </x-form-row>
    </x-form-section>
        ...
        <x-form-row>
            <x-form-field label="Page Content" name="content" :required="true" span="full">
                <x-textarea name="content" rows="12" :required="true" 
                           placeholder="Write your page content here...">{{ old('content') }}</x-textarea>
            </x-form-field>
        </x-form-row>
    </x-form-section>
    ...
    <div class="flex items-center gap-3 pb-6">
        <x-btn type="submit">Create Page</x-btn>
