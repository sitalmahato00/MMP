...
@section('title', 'Create Assessment Exam')

@section('content')
<x-page-header title="Create Assessment Exam" subtitle="Create a new monthly assessment exam for your department."
               back="{{ route('hod.exams.index') }}"/>

<form method="POST" action="{{ route('hod.exams.store') }}" 
      class="max-w-4xl space-y-6"
      x-data="{
            programs: [{ program_id: '', semester: 1 }],
            addProgram() {
        ...
        <x-form-row>
            ...
            <x-form-field label="Assessment Number" name="assessment_number" :required="true">
                <x-select name="assessment_number" :required="true">
                    ...
                    @for($i = 1; $i <= 12; $i++)
                        <option value="{{ $i }}" @selected(old('assessment_number') == $i)>
                            Assessment {{ $i }}
                        </option>
                    @endfor
                </x-select>
            ...
            <x-form-field label="Full Marks" name="assessment_full_marks" :required="true">
                <x-input type="number" name="assessment_full_marks" :value="old('assessment_full_marks', 100)" 
                         :required="true" step="0.01" min="0" placeholder="100"/>
            </x-form-field>

            <x-form-field label="Pass Marks" name="assessment_pass_marks" :required="true">
                <x-input type="number" name="assessment_pass_marks" :value="old('assessment_pass_marks', 40)" 
    ...
    <x-form-section title="Schedule" subtitle="Exam start and end dates in Bikram Sambat.">
        <x-form-row>
            <x-form-field label="Start Date (BS)" name="start_date_bs" :required="true">
                <x-bs-date-picker name="start_date_bs" :value="old('start_date_bs')" :required="true"/>
            </x-form-field>

            <x-form-field label="End Date (BS)" name="end_date_bs">
                <x-bs-date-picker name="end_date_bs" :value="old('end_date_bs')"/>
        ...
        <div class="space-y-3">
            <template x-for="(program, index) in programs" :key="index">
                <div class="flex gap-3 items-start">
                    <div class="flex-1">
                        <select :name="`programs[${index}]`" required
                    ...
                    <div class="w-40">
                        <select :name="`semesters[${index}]`" required
                                class="block w-full rounded-lg border-slate-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm">
    ...
    <div class="flex items-center gap-3 pb-6">
        <x-btn type="submit">Create Assessment Exam</x-btn>
