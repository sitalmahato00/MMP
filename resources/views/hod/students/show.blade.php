...
@php
    ...
    $attendancePctColor = $attendanceRate === null ? 'text-slate-500' : ($attendanceRate >= 75 ? 'text-emerald-600' : ($attendanceRate >= 50 ? 'text-amber-600' : 'text-red-600'));
@endphp
    ...
    <div class="relative px-6 py-7">
        <div class="flex flex-wrap items-start gap-5">
            ...
            <div class="min-w-0 flex-1">
                ...
                <p class="mt-1 font-mono text-sm text-blue-100">{{ $student->student_no }}</p>
        ...
        <div class="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-4">
            @php
                $pctColor2 = $attendanceRate === null ? 'text-blue-100' : ($attendanceRate >= 75 ? 'text-emerald-300' : ($attendanceRate >= 50 ? 'text-amber-300' : 'text-red-300'));
            @endphp
            <div class="rounded-xl bg-white/10 p-3 text-center">
                <p class="text-2xl font-black {{ $pctColor2 }}">{{ $attendanceRate !== null ? $attendanceRate.'%' : '—' }}</p>
                <p class="mt-0.5 text-[11px] text-blue-200">Attendance</p>
            </div>
            <div class="rounded-xl bg-white/10 p-3 text-center">
                <p class="text-2xl font-black text-white">{{ $marks->count() }}</p>
                <p class="mt-0.5 text-[11px] text-blue-200">Exam records</p>
            </div>
            <div class="rounded-xl bg-white/10 p-3 text-center">
                <p class="text-2xl font-black text-white">{{ $assignments->where('submission_id', '!=', null)->count() }}</p>
                <p class="mt-0.5 text-[11px] text-blue-200">Submissions</p>
            </div>
            <div class="rounded-xl bg-white/10 p-3 text-center">
                <p class="text-2xl font-black text-white">{{ $student->parents->count() }}</p>
                <p class="mt-0.5 text-[11px] text-blue-200">Parent accounts</p>
            </div>
        </div>
    </div>
</div>
...
<div x-show="tab === 'attendance'" class="space-y-5">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
        ...
        <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm text-center">
            @php $attPct = $attendanceRate; $attClr = $attPct === null ? 'text-slate-500' : ($attPct >= 75 ? 'text-emerald-600' : ($attPct >= 50 ? 'text-amber-600' : 'text-red-600')); @endphp
    ...
    <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <div class="flex items-center justify-between mb-3">
            ...
            <span class="text-sm font-black {{ $attClr }}">{{ $attendanceRate }}%</span>
        </div>
        <div class="h-4 w-full rounded-full bg-slate-100 overflow-hidden">
            <div class="h-4 rounded-full transition-all duration-700 {{ $attendanceRate >= 75 ? 'bg-emerald-500' : ($attendanceRate >= 50 ? 'bg-amber-400' : 'bg-red-500') }}"
                 style="width: {{ $attendanceRate }}%"></div>
        </div>
        <div class="mt-2 flex items-center justify-between text-xs text-slate-400">
            <span>0%</span>
            <span class="{{ $attendanceRate < 75 ? 'text-amber-600 font-semibold' : '' }}">
                {{ $attendanceRate < 75 ? '⚠ Below 75% minimum required' : '✓ Above minimum threshold' }}
            </span>
            <span>100%</span>
        </div>
    </div>
    ...
    <div class="rounded-2xl border border-slate-200 bg-white overflow-hidden shadow-sm">
        <div class="bg-slate-50 border-b border-slate-200 px-5 py-3">
            <h3 class="text-sm font-bold text-slate-700">Monthly Breakdown (Last 6 Months)</h3>
        </div>
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-slate-100">
                ...
                @php 
                    $mp = $row['total'] > 0 ? round(($row['present'] / $row['total']) * 100) : 0; 
                @endphp
...
<div x-show="tab === 'marks'" class="space-y-5">
    ...
    @else
    @foreach($examsBySemester as $semester => $semesterMarks)
    <div class="rounded-2xl border border-slate-200 bg-white overflow-hidden shadow-sm">
        <div class="flex items-center gap-3 border-b border-slate-100 bg-slate-50/60 px-5 py-3.5">
            ...
            <span class="text-xs text-slate-400">{{ $semesterMarks->count() }} record(s)</span>
        </div>
            ...
            <table class="w-full text-sm">
                <thead class="border-b border-slate-100">
                    <tr>
                        ...
                        <th class="px-5 py-3 text-center text-[11px] font-bold uppercase tracking-wider text-slate-500">Theory</th>
                        <th class="px-5 py-3 text-center text-[11px] font-bold uppercase tracking-wider text-slate-500">Practical</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    @php
                        $theoryTotal = ($mark->internal_theory_marks ?? 0) + ($mark->external_theory_marks ?? 0);
                        $practicalTotal = ($mark->internal_practical_marks ?? 0) + ($mark->external_practical_marks ?? 0);
                        $total = $theoryTotal + $practicalTotal;
                        $isPassing = $total >= 40;
                    @endphp
                    <tr class="hover:bg-slate-50/60">
                        <td class="px-5 py-3 font-medium text-slate-800">{{ $mark->subject_name }}</td>
                        <td class="px-5 py-3 text-xs text-slate-500">{{ $mark->exam_name ?? '—' }}</td>
                        <td class="px-5 py-3 text-center text-slate-700">{{ $theoryTotal > 0 ? number_format($theoryTotal, 1) : '—' }}</td>
                        <td class="px-5 py-3 text-center text-slate-700">{{ $practicalTotal > 0 ? number_format($practicalTotal, 1) : '—' }}</td>
                        <td class="px-5 py-3 text-center font-bold text-slate-900">{{ number_format($total, 1) }}</td>
                        <td class="px-5 py-3 text-center">
                            <span class="rounded-full px-2.5 py-1 text-[11px] font-bold {{ $isPassing ? 'bg-emerald-50 text-emerald-700' : 'bg-red-50 text-red-600' }}">
                                {{ $isPassing ? 'Pass' : 'Fail' }}
                            </span>
                        </td>
                    </tr>
                ...
                <tfoot class="border-t border-slate-200 bg-slate-50/50">
                    @php
                        $semTotal = $semesterMarks->sum(function($m) {
                            return ($m->internal_theory_marks ?? 0) + ($m->external_theory_marks ?? 0) +
                                   ($m->internal_practical_marks ?? 0) + ($m->external_practical_marks ?? 0);
                        });
                    @endphp
                    <tr>
                        <td colspan="4" class="px-5 py-2.5 text-right text-xs font-bold text-slate-500 uppercase tracking-wider">Semester Total</td>
...
<div x-show="tab === 'assignments'" class="space-y-4">
    ...
    <div class="flex flex-col items-center justify-center rounded-2xl border border-slate-200 bg-white py-16 text-slate-400 shadow-sm">
        ...
        <p class="text-sm font-medium">No assignments for current semester</p>
    </div>
    @else
    <div class="rounded-2xl border border-slate-200 bg-white overflow-hidden shadow-sm">
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-slate-100">
                ...
                @php
                    ...
                    if ($assignment->submission_id) {
                        $status = $assignment->submission_status ?? 'submitted';
                ...
                @endphp
                <tr class="hover:bg-slate-50/60">
                    <td class="px-5 py-3 font-medium text-slate-800 max-w-[200px] truncate">{{ $assignment->title ?? '—' }}</td>
                    <td class="px-5 py-3 text-xs text-slate-500">{{ $assignment->subject_name ?? '—' }}</td>
                    <td class="px-5 py-3 text-xs text-slate-500 hidden sm:table-cell">
                        {{ $assignment->due_date ? bsDate($assignment->due_date, 'Y, F d') : '—' }}
                    </td>
                    <td class="px-5 py-3 text-center">
                        <span class="rounded-lg px-2.5 py-1 text-[11px] font-bold {{ $statusCls }}">{{ ucfirst($status) }}</span>
                    </td>
                    <td class="px-5 py-3 text-center font-bold text-slate-700">
                        {{ $assignment->obtained_marks !== null ? number_format($assignment->obtained_marks, 1) : '—' }}
                    </td>
                </tr>
                @endforeach
            </tbody>
...
<div x-show="tab === 'parent'" class="space-y-4">
    ...
    <div class="flex flex-col items-center justify-center rounded-2xl border border-slate-200 bg-white py-16 text-slate-400 shadow-sm">
        ...
        <a href="{{ route('hod.students.edit', $student) }}"
           class="mt-3 inline-flex items-center gap-1.5 rounded-xl bg-[#1d4ed8] px-4 py-2 text-xs font-bold text-white hover:bg-[#1e40af] transition">
