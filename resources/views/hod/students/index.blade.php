...
<div x-data="{
    view: localStorage.getItem('mmp_hod_students_view') ?? 'table',
    selected: [],
    drawer: false,
    drawerLoading: false,
    drawerHtml: '',
    drawerStudentId: null,
    setView(v) { this.view = v; localStorage.setItem('mmp_hod_students_view', v); },
    toggleAll(ids) {
...
<div class="flex flex-wrap items-start justify-between gap-4">
    <div>
        <h1 class="text-2xl font-black tracking-tight text-slate-900">Department Students</h1>
        <p class="mt-0.5 text-sm text-slate-500">
            {{ $department->name }} — enroll, edit and manage students in your department
        </p>
    </div>
    <div class="flex flex-wrap items-center gap-2">
        {{-- Export Component --}}
        <x-export-dropdown 
            :export-url="route('hod.students.export')"
            :query-params="request()->query()"
            :formats="['csv', 'excel', 'pdf']"
            button-text="Export Students"
            button-class="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-600 shadow-sm hover:bg-slate-50 transition" />
...
{{-- ── FILTERS ─────────────────────────────────────────────── --}}
<form method="GET" action="{{ route('hod.students.index') }}"
    ...
    <div class="grid gap-3 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
        ...
        <div class="flex gap-2">
            ...
            @if(request()->hasAny(['search','program_id','semester','status','academic_session_id']))
        ...
        <div class="flex items-center gap-3 min-w-0">
            <template x-if="selected.length > 0">
                <div class="flex items-center gap-2">
                    ...
                    <button type="button" class="rounded-lg border border-violet-200 bg-violet-50 px-3 py-1.5 text-xs font-semibold text-violet-700 hover:bg-violet-100 transition">Promote</button>
    ...
    <div x-show="view === 'table'" x-cloak>
        @if($students->isEmpty())
            <div class="flex flex-col items-center justify-center py-20 text-center">
                ...
                <a href="{{ route('hod.students.create') }}" class="mt-4 inline-flex items-center gap-2 rounded-xl bg-[#1d4ed8] px-4 py-2 text-sm font-bold text-white hover:bg-[#1e40af] transition">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"/></svg>
                    Enroll Student
                </a>
            </div>
            ...
            <table class="mmp-table w-full text-sm">
                    ...
                    <tr class="bg-slate-50/70 border-b border-slate-100">
                        <th class="w-10 px-5 py-3 text-left">
                            <input type="checkbox" @change="toggleAll({{ $allIds }})"
                                   ...
                                   class="h-4 w-4 rounded border-slate-300 text-[#1d4ed8] focus:ring-[#1d4ed8]"/>
                        </th>
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr class="group hover:bg-slate-50/60 transition-colors"
                        :class="selected.includes({{ $student->id }}) ? 'bg-blue-50/30' : ''">
                            ...
                            <div class="flex items-center gap-3 min-w-0">
                                ...
                                <div class="min-w-0">
                                    <button type="button" @click="openDrawer({{ $student->id }})"
                                            class="block font-semibold text-slate-900 hover:text-[#1d4ed8] truncate transition text-sm text-left">{{ $student->user?->name }}</button>
                                    <p class="font-mono text-[11px] text-slate-400 truncate">{{ $student->student_no ?? '—' }}</p>
                                    @if($student->roll_number)
                                        <p class="font-mono text-[11px] text-blue-600 truncate">Roll: {{ $student->roll_number }}</p>
                                    @endif
                        ...
                        <td class="px-5 py-3.5">
                            ...
                            <p class="text-xs text-slate-400 truncate">{{ $student->section ? 'Section ' . $student->section : 'No section' }}</p>
                        </td>
                        <td class="px-5 py-3.5">
                        ...
                        <td class="px-5 py-3.5">
                            <div class="flex items-center justify-end gap-1">
                                ...
                                <form method="POST" action="{{ route('hod.students.destroy', $student) }}"
                                      onsubmit="return confirm('Delete {{ addslashes($student->user?->name ?? 'this student') }}? This cannot be undone.')">
                                    @csrf @method('DELETE')
                                    <button type="submit"
        ...
        <div class="grid gap-4 p-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            ...
            <div class="group relative rounded-2xl border border-slate-200 bg-white p-5 shadow-sm hover:shadow-md hover:border-slate-300 transition-all duration-150">
                ...
                <div class="flex flex-col items-center text-center">
                    ...
                    @endif
                    <button type="button" @click="openDrawer({{ $student->id }})"
                            class="mt-3 text-sm font-bold text-slate-900 hover:text-[#1d4ed8] transition leading-tight pr-5 text-center">{{ $student->user?->name }}</button>
                    <p class="mt-0.5 font-mono text-[11px] text-slate-400">{{ $student->student_no ?? '—' }}</p>
                    @if($student->roll_number)
                        <p class="font-mono text-[11px] text-blue-600">Roll: {{ $student->roll_number }}</p>
                    @endif
                </div>
                {{-- Badges --}}
                ...
                <div class="mt-3 space-y-0.5 text-center">
                    ...
                    <p class="text-[11px] text-slate-400 truncate">{{ $student->section ? 'Section ' . $student->section : 'No section' }}</p>
...
<div x-show="drawer"
     ...
     class="fixed inset-y-0 right-0 z-50 w-full max-w-2xl bg-white shadow-2xl"
     x-cloak>
