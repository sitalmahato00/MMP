...
<body class="bg-gray-50 flex items-center justify-center min-h-screen p-4" style="background-image: url('{{ asset('assets/bg-pattern.png') }}'); background-blend-mode: overlay; background-color: #f9fafb;">
    <div class="w-full max-w-md bg-white rounded-md shadow-[0_8px_30px_rgb(0,0,0,0.12)] overflow-hidden border-t-4 border-[#8B0000]">
        <div class="p-8">
            ...
            <form method="POST" action="{{ route('login') }}" class="space-y-5">
                ...
                <div>
                    ...
                    <input type="email" name="email" value="{{ old('email') }}" required autofocus class="w-full rounded border-gray-300 focus:border-[#8B0000] focus:ring-[#8B0000] shadow-sm py-2.5 px-3 border transition-colors outline-none text-sm">
                </div>

                <div>
                    <div class="flex items-center justify-between mb-1.5">
                        <label class="block text-sm font-bold text-gray-700">Password</label>
                        <a href="#" class="text-xs font-bold text-[#8B0000] hover:text-red-900 hover:underline">Forgot password?</a>
                    </div>
                    <input type="password" name="password" required class="w-full rounded border-gray-300 focus:border-[#8B0000] focus:ring-[#8B0000] shadow-sm py-2.5 px-3 border transition-colors outline-none text-sm">
                </div>

                <div class="flex items-center">
                    <input type="checkbox" name="remember" id="remember" class="w-4 h-4 text-[#8B0000] rounded border-gray-300 focus:ring-[#8B0000]">
                    <label for="remember" class="ml-2 block text-sm font-medium text-gray-700">Remember me</label>
                </div>

                <button type="submit" class="w-full bg-[#8B0000] hover:bg-[#6D0000] text-white font-bold py-3 px-4 rounded shadow-md transition-all active:scale-[0.98] outline-none focus:ring-2 focus:ring-[#8B0000] focus:ring-offset-2 uppercase tracking-wider text-sm mt-2">
                    Sign in to Account
                </button>
            </form>
            
            <div class="mt-8 pt-6 border-t border-gray-100 flex items-center justify-between">
                <a href="{{ route('home') }}" class="text-xs font-bold text-gray-500 hover:text-[#8B0000] flex items-center gap-1 transition-colors">
