@php
    ...
    $academicYear  = $siteSettings['academic_year'] ?? null;
...
<header class="fixed inset-x-0 top-0 z-40 flex h-16 items-center justify-between gap-4 px-4 sm:px-5 lg:px-6"
    ...
    {{-- ── CENTER: Academic Year + Session badge ──────────────────────── --}}
    <div class="hidden md:flex items-center justify-center gap-2 flex-shrink-0">
        @if($academicYear)
            <span class="inline-flex items-center gap-1.5 rounded px-3 py-1.5 text-sm font-bold"
                  style="background-color: rgba(234,179,8,0.18); color: #fde68a; border: 1px solid rgba(234,179,8,0.3);">
    ...
    <div class="flex items-center gap-1 sm:gap-2 flex-shrink-0">
        ...
        <div class="relative" x-data="{ notificationsOpen: false }">
            ...
            <div x-show="notificationsOpen"
                 ...
                 class="absolute right-0 mt-2 w-80 overflow-hidden bg-white dark:bg-[#132044] shadow-xl"
                ...
                <div class="flex items-center justify-between px-4 py-3" style="border-bottom: 1px solid #F0F3F7;">
                    ...
                    <a href="{{ route('notifications.index') }}" class="text-xs font-semibold text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300">View all</a>
                </div>
                @if($headerNotifications->isEmpty())
                    <div class="px-5 py-8 text-center">
                        <div class="mx-auto flex h-10 w-10 items-center justify-center rounded bg-[#F4F7FB] dark:bg-[#1e3a5f]">
                            <svg class="h-5 w-5 text-gray-400 dark:text-slate-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.9" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9"/>
                            </svg>
                        </div>
                        <p class="mt-3 text-sm font-semibold text-gray-800 dark:text-slate-200">No notifications</p>
                        <p class="mt-1 text-xs text-gray-500 dark:text-slate-400">Official updates will appear here.</p>
                    </div>
                @else
                    <div class="max-h-[24rem] divide-y overflow-y-auto" style="border-color: #F0F3F7;">
                        @foreach($headerNotifications as $notification)
                            ...
                            <a href="{{ route('notifications.open', $notification) }}"
                               class="block px-4 py-3 transition hover:bg-gray-50 dark:hover:bg-[#1e3a5f] {{ $nUnread ? 'bg-blue-50/40 dark:bg-blue-900/10' : '' }}">
                                <div class="flex items-start gap-3">
                                    <span class="mt-1.5 h-2 w-2 flex-shrink-0 rounded-full {{ $nUnread ? 'bg-blue-500' : 'bg-gray-300 dark:bg-slate-600' }}"></span>
                ...
                @endif
                <div class="px-4 py-2.5 dark:bg-[#0D1B35]" style="border-top: 1px solid #F0F3F7; background-color: #F8FAFC;">
        ...
        <div class="relative" x-data="{ userMenuOpen: false }">
            ...
            <div x-show="userMenuOpen"
                 x-transition.opacity.duration.150ms
                 class="absolute right-0 z-50 mt-2 w-64 overflow-hidden bg-white dark:bg-[#132044] shadow-xl"
                 style="border: 1px solid #DCE3EB; border-radius: 4px;"
                 x-cloak>

                <!-- Header -->
                <div class="px-4 py-3 dark:bg-[#1a2f50]" style="background-color: #F4F7FB; border-bottom: 1px solid #DCE3EB;">
                ...
                <a href="{{ $settingsRoute }}"
                   class="flex items-center gap-2.5 px-4 py-2.5 text-sm text-gray-700 dark:text-slate-300 transition-colors hover:bg-gray-50 dark:hover:bg-[#1e3a5f] hover:text-blue-700 dark:hover:text-blue-400">
                ...
                <div style="border-top: 1px solid #F0F3F7;" class="dark:border-[#1e3a5f]"></div>

                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button type="submit"
                            class="flex w-full items-center gap-2.5 px-4 py-2.5 text-sm transition-colors hover:bg-red-50 dark:hover:bg-red-900/20"
                            style="color: #dc2626;">
                        <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
