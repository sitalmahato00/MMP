...
export default function UsersPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['system-users', search, role, page],
    queryFn: () => {
      const p = new URLSearchParams({ page: String(page), per_page: '20' });
      if (search) p.set('search', search);
      if (role) p.set('role', role);
      return get<any>(`/v1/users?${p}`);
    },
  });
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-black tracking-tight text-slate-900">System Users</h1>
          <p className="mt-0.5 text-sm text-slate-500">All portal accounts across every role.</p>
        </div>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm flex flex-wrap gap-3">
        ...
        <select value={role} onChange={e => { setRole(e.target.value); setPage(1); }}
          ...
          {Object.entries(ROLE_MAP).map(([v, s]) => <option key={v} value={v}>{s.label}</option>)}
        </select>
      </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-slate-50/70 border-b border-slate-100">
                <tr>{['User','Role','Status','Created','Actions'].map(h => (
                  <th key={h} className="px-5 py-3 text-left text-[11px] font-bold uppercase tracking-wider text-slate-500">{h}</th>
                ))}</tr>
              </thead>
              <tbody className="divide-y divide-slate-100">
                {users.map((u: any) => {
                  const rm = ROLE_MAP[u.role] ?? { label: u.role, cls: 'bg-slate-100 text-slate-600' };
                  return (
                    <tr key={u.id} className="group hover:bg-slate-50/60 transition-colors">
                      <td className="px-5 py-3.5">
                        <div className="flex items-center gap-3">
                          {u.avatar
                            ? <img src={u.avatar} className="h-9 w-9 rounded-xl object-cover flex-shrink-0" alt={u.name} />
                            : <div className="flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-xl bg-blue-100 text-blue-700 text-sm font-black">{u.name?.charAt(0)?.toUpperCase()}</div>}
                          <div>
                      ...
                      <td className="px-5 py-3.5"><span className={`rounded-lg px-2.5 py-1 text-xs font-bold ${rm.cls}`}>{rm.label}</span></td>
                      <td className="px-5 py-3.5">
                        <span className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-semibold ${u.is_active ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-500'}`}>
                          <span className={`h-1.5 w-1.5 rounded-full ${u.is_active ? 'bg-emerald-500' : 'bg-slate-400'}`} />{u.is_active ? 'Active' : 'Inactive'}
                        </span>
                      </td>
                      <td className="px-5 py-3.5 text-xs text-slate-400"><BsDate date={u.created_at} format="Y, F d" /></td>
                      <td className="px-5 py-3.5">
                        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                          <Link to={`${u.id}/edit`} className="inline-flex h-8 w-8 items-center justify-center rounded-lg text-slate-400 hover:bg-amber-50 hover:text-amber-600 transition" title="Edit">
